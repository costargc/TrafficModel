...
for(l=1;l<=6;l++){//Vm loop start
	Vm=Vs[l]; // need to change to l letter L lowercase
	int n=Nc[l]; //searching for phase transition...

char fname1[30]="NaSch_int_seq_rho_";
char buf1[100000];
sprintf(buf1, "%s%d_ds3.dat", fname1, Vm);
FILE *f1;
f1 = fopen(buf1,"w");
...
int t=1; //time searching phase transition
 ...
 while(t<=600){ //searching or phase transition
	 ...
	 flux=0;
	  ...
	  /*only used when generating the Probability map
	  int j;
	  long double P000=0;
	  long double P001=0;
	  long double P010=0;
...
if(sd<=3){ // if sd<=0, its the case with no particle interaction!
		  v[select]=fmin(fmin(fmin(Vm,sd),v[select]+1),v[next]); //select velocity to update
		 // v[select]=fmin(fmin(Vm,sd),v[next]); //another option with maximum acceleration!!!
	  }
	  else{
...
//headway map generation attempt
